...
### 9. 感知色彩空間誤差評估 (Weighted RGB MSE) — 品質優化回歸
* **原理說明**：在 GPU 開放運算 (OpenCL) 核函式中，使用感知色彩通道權重。綠色通道給予最高加權 (0.4f)，而紅色與藍色通道的權重則根據像素亮度動態調整。
* **說明**：v3 版本經過重新評估，確認此機制對於人像主題（精確對比與亮度感知）具有明確的正面貢獻。目前已將 Go 端與 OpenCL Kernel 完整還原為此感知加權設定，以提供更逼真的視覺細節。

### 10. 影像前處理管線 (Source Pre-Processing Pipeline)
* **Alpha 邊緣去噪 (Alpha Fringe Removal)**：針對去背去邊緣鋸齒（邊緣不透明像素）進行 7x7 圓形膨脹與硬落格判定，過濾 Alpha 微弱雜訊。
* **硬邊緣 Logo 修復 (Logo Hard Edges)**：對去背圖片邊緣半透明區域進行最近鄰實色覆蓋（BFS 距離場），並將有色像素之 Alpha 鎖定為 1.0，防止遊戲內產生邊緣白邊雜質。
* **Luma Bands 漸層優化**：利用 LAB 色彩空間量化 L 通道，計算 Sobel 邊緣強度權重，限制隨機取樣時對漸層背景的誤差干擾，極大化單一色塊的拼合平滑度。

### 11. 進階重要性權重圖 (Advanced Importance Map)
* **動態複合權重計算**：結合亮度、飽和度、Alpha 透明邊緣與高光/暗部線條，生成最高 5.25 倍加權的動態重要性地圖。
* **GPU OpenCL 內聯加權**：將複合權重地圖完全上載至 GPU，在 OpenCL 核心比對的像素點累加（Pixel-loop Reduction）階段直接進行內聯加權。與以往的後處理或外置 Edge Map 不同，此方法能完美導向優化器向關鍵邊緣及線條極速收斂，避免分數抖動。

### 12. 進階智慧取樣策略 (Advanced Sampling)
* **兩階段隨機加速 (Two-Stage Random)**：預設啟用。第一階段以跳步步長（`SampleStep=2`）在 200,000 個候選中進行粗篩選，精挑最優的 2048 個候選以 `SampleStep=1` 在第二階段精細評估，使高畫質下幾十萬級別的候選篩選能在數百毫秒內完成。
* **後期微型候選 (Late Small Candidates)**：在生成進度達 50% 後，將 66% 的隨機候選橢圓半徑強制限制為微小比例（`maxRadius * 0.024`），精確修補字體邊角、人物眼部與細節線條。
* **自適應圖像特徵分析 (Adaptive Profiler)**：啟動時自動解析來源圖像的透明覆蓋率、邊緣密度、亮度和純白像素特徵，於主控台輸出分類建議並調整最佳前處理設定。

### 13. 後處理優化與智慧裁剪 (Post-Processing & Pruning)
* **貢獻度智慧裁剪 (Contribution-Based Pruning)**：主迴圈結束後，利用 CPU $O(N)$ 單次渲染遍歷逆向評估所有圖層的真實 MSE 誤差貢獻值。以 `PruneThreshold = 1e-7` 閥值精準剔除無效、負貢獻或完全遮擋的冗餘圖層，減少 Forza 車貼圖層耗用。
* **幾何尺寸補償 (Ellipse Size Compensation)**：對輸出的橢圓形半徑 `RX` 與 `RY` 自動乘以 `1.0125` 的補償因子。用以對齊《極限競速》遊戲內渲染大圓/高拉伸橢圓時的實質尺寸膨脹現象，確保本地擬合效果與遊戲實機無損一致。

*(註：已於 v3 版本移除先前加入的 CMA-ES 早停機制，確保所有圖層皆獲得完整的迭代優化，保障最終的細節表現。)*
