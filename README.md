...
### 11. 前端可選式圖片預處理 (Optional Image Preprocessing)
* **原理說明**：在幾何擬合前，使用者可啟用預處理功能，利用前端 HTML5 Canvas 即時對圖片進行**雙邊平滑去噪 (Bilateral Smoothing)**與**色階量化/海報化 (Posterize)**。
* **說明**：畫面越扁平、色階越少，引擎的橢圓擬合效率與邊緣完整度就越高。此功能為**可選方案**，使用者可在上傳圖片後即時調整平滑度與色階，滿意後再提交給後端開始生成。
