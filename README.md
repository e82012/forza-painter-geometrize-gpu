...
### 9. 感知色彩空間誤差評估 (Weighted RGB MSE)
* **原理說明**：在 GPU 開放運算 (OpenCL) 核函式中，捨棄傳統均勻 RGB MSE，改用感知色彩通道權重。根據人眼對不同顏色的視覺敏感度，綠色通道給予最高加權 (0.4f)，而紅色與藍色通道的權重則根據像素亮度（R通道亮度）動態調整（R通道為 `0.2f + 0.1f * R_avg`，B通道為 `0.3f + 0.1f * (1.0f - R_avg)`）。
* **說明**：本特性在不破壞 OpenCL 的解析式單次走訪 (Analytic Single-Pass) 優化公式的前提下，大幅提升了對色彩漸層、人臉皮膚以及暗部細節的擬合逼真度與細膩感。

### 10. 自適應邊緣引導權重衰減 (Adaptive Edge Weight Decay)
* **原理說明**：`EdgeWeight` 邊緣引導權重不再是固定常數。擬合前期，大面積形狀不需要精確貼邊，權重設定為較低的 0.2x，以防過早被局部邊緣「重力」吸引至錯誤位置；隨著擬合進度推移（progress 增加），邊緣引導權重逐步放大到 1.0x。
* **說明**：最大化後期小尺寸形狀對輪廓、文字與細節邊緣的擬合貼合率。
